...
class WebHelper
{
     ...
     *
     */
    public static function changeDateNow()
    {
     ...
     *
     */
    public static function apiTelegramNotification(String $token, String $chat_id, String $msg)
    {
        try {
            return Http::timeout(3)->withOptions([
                'debug' => fopen('php://stderr', 'w'),
                'verify' => false,
            ])->get("https://api.telegram.org/bot{$token}/sendMessage?chat_id={$chat_id}&text={$msg}&parse_mode=html");
        } catch (\Exception $e) {
            Log::info($e->getMessage());
        }
    }
    ...
    public static function search_array($needle, $haystack)
    {
        foreach ($haystack as $key => $value) {
            if (is_array($value)) {
                $result = WebHelper::search_array($needle, $value);
                if ($result !== false) {
                    return $result;
                }
            } else {
     ...
     *
     */
    public static function rename_role($str)
    {
        $palavras_substituicao = array(
            'bairro' => 'bairro',
        ...
        foreach ($palavras_substituicao as $palavra_original => $palavra_substituta) {
            // Substituir a palavra original pela palavra substituta no texto
            $str = str_replace($palavra_original, $palavra_substituta, $str);
        }
